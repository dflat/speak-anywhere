...
private:
    ...
    std::expected<void, std::string> terminal_paste(const std::string& text, WindowManager& win);
    std::expected<void, std::string> general_paste(const std::string& text, WindowManager& win);
};
